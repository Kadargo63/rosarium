...
export function GardenManager({ initialGardens }: { initialGardens: Garden[] }) {
  ...
  const set = (key: keyof GardenFormData, val: string) => setForm((f) => ({ ...f, [key]: val }))
  ...
  const FormFields = ({ isNew }: { isNew: boolean }) => (
    <div className="space-y-3 border rounded-xl p-4 bg-neutral-50">
        ...
        <div className="mt-1 grid grid-cols-1 gap-1.5">
          {SUN_OPTIONS.map((s) => (
            <button key={s.value} type="button"
              onClick={() => set('sun_index', form.sun_index === s.value ? '' : s.value)}
      ...
      <div className="flex gap-2 pt-1">
        <button onClick={() => save(isNew)} disabled={saving}
        ...
        <button onClick={() => { setShowAdd(false); setEditId(null); setForm(emptyForm()) }}
          className="px-4 py-2.5 border rounded-xl text-sm text-neutral-600 hover:bg-neutral-100">
            ...
            <div className="bg-white border rounded-xl p-4">
              <div className="flex items-start justify-between gap-2">
                    ...
                    <div className="flex items-center gap-2 mt-1.5">
                      ...
                      {g.sun_index && (
                        <span className="text-xs text-neutral-400">
                          · {SUN_OPTIONS.find((s) => s.value === g.sun_index)?.label ?? g.sun_index}
                        </span>
                      )}
                    </div>
                  </div>
                ...
                <div className="flex gap-1 flex-shrink-0">
                  ...
                  <button onClick={() => setDeleteId(g.id === deleteId ? null : g.id)}
              ...
              {deleteId === g.id && (
                <div className="mt-3 p-3 bg-red-50 border border-red-200 rounded-lg space-y-2">
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => confirmDelete(g.id)} disabled={saving}
                      className="flex-1 bg-red-600 hover:bg-red-700 disabled:opacity-60 text-white text-xs font-medium py-2 rounded-lg">
                      {saving ? 'Deleting…' : 'Yes, delete'}
                    </button>
                    <button onClick={() => setDeleteId(null)}
                      className="flex-1 border text-xs text-neutral-600 py-2 rounded-lg hover:bg-neutral-50">
                      Cancel
                    </button>
                  </div>
                </div>
